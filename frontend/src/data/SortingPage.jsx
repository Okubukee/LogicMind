...
import DualSortingVisualizer from '../../components/DualSortingVisualizer';
...
const SortingPage = () => {
  ...
  const [rightAlgorithm, setRightAlgorithm] = useState('bitonic');
  const [leftSteps, setLeftSteps] = useState([]);
  const [rightSteps, setRightSteps] = useState([]);
  const [leftStepIndex, setLeftStepIndex] = useState(0);
  const [rightStepIndex, setRightStepIndex] = useState(0);
  const [isDualPlaying, setIsDualPlaying] = useState(false);
  ...
  const rightStepsRef = useRef([]);
  const leftStepIndexRef = useRef(0);
  const rightStepIndexRef = useRef(0);
  const isDualPlayingRef = useRef(false);
  ...
  useEffect(() => {
    ...
    leftStepsRef.current = leftSteps;
    rightStepsRef.current = rightSteps;
    leftStepIndexRef.current = leftStepIndex;
    rightStepIndexRef.current = rightStepIndex;
    isDualPlayingRef.current = isDualPlaying;
  });

  
  useEffect(() => {
    if (activeView === 'single' && isPlayingRef.current && stepsRef.current.length > 0) {
    ...
    if (activeView === 'dual' && isDualPlayingRef.current && leftStepsRef.current.length > 0) {
      if (intervalRef.current) clearInterval(intervalRef.current);
      const leftData = leftStepsRef.current;
      const rightData = rightStepsRef.current;
      let leftIdx = leftStepIndexRef.current;
      let rightIdx = rightStepIndexRef.current;
      const getDelay = () => {
        const option = speedOptions.find(opt => opt.value === speedMultiplierRef.current);
        return option ? option.delay : 500;
      };
      const animate = () => {
        let anyActive = false;
        if (leftIdx < leftData.length - 1) {
          leftIdx++;
          setLeftStepIndex(leftIdx);
          anyActive = true;
        }
        if (rightIdx < rightData.length - 1) {
          rightIdx++;
          setRightStepIndex(rightIdx);
          anyActive = true;
        }
        if (!anyActive) {
          if (intervalRef.current) clearInterval(intervalRef.current);
          setIsDualPlaying(false);
        }
      };
      intervalRef.current = setInterval(animate, getDelay());
    }
  }, [speedMultiplier, activeView]);
  ...
  const getAlgorithmIcon = (algoId) => {
    return algorithmsInfo[algoId]?.icon || '📊';
  };

  const getAlgorithmName = (algoId) => {
    return algorithmsInfo[algoId]?.name || algoId;
  };
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '32px', display: 'flex', justifyContent: 'center' }}>
          <div style={{ 
            ...
            <button 
              onClick={() => { setActiveView('single'); stopAnimation(); }}
              style={{
                padding: '12px 28px',
                borderRadius: '40px',
                border: 'none',
                background: activeView === 'single' ? 'linear-gradient(135deg, #00ff9d, #10b981)' : 'rgba(255,255,255,0.1)',
                color: activeView === 'single' ? '#000000' : '#ffffff',
                fontSize: '15px',
                fontWeight: 'bold',
                cursor: 'pointer'
              }}
        ...
        {activeView === 'single' && (
          <>
            ...
            <div style={{ background: 'rgba(20, 20, 20, 0.9)', borderRadius: '28px', padding: '28px', marginBottom: '32px', border: '1px solid rgba(0, 255, 157, 0.2)' }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center', marginBottom: '20px' }}>
                <select value={algorithm} onChange={(e) => { setAlgorithm(e.target.value); setSteps([]); setCurrentStep(0); }} disabled={isPlaying}
                  style={{ background: '#1a1a1a', color: 'white', border: '1px solid #00ff9d', borderRadius: '12px', padding: '10px 14px' }}>
                  {algorithmsList.map(algoId => (
                    <option key={algoId} value={algoId}>{getAlgorithmIcon(algoId)} {getAlgorithmName(algoId)}</option>
                  ))}
                </select>
                <button onClick={generateRandomArray} style={{ padding: '12px 24px', borderRadius: '16px', background: 'linear-gradient(135deg, #00ff9d, #10b981)', color: '#000', fontWeight: 'bold', border: 'none', cursor: 'pointer' }}>🎲 Aleatorio</button>
                <button onClick={visualizeSort} style={{ padding: '12px 24px', borderRadius: '16px', background: 'linear-gradient(135deg, #00ff9d, #10b981)', color: '#000', fontWeight: 'bold', border: 'none', cursor: 'pointer' }}>▶ Visualizar</button>
                {isPlaying && <button onClick={stopAnimation} style={{ padding: '12px 24px', borderRadius: '16px', background: '#ef4444', color: 'white', border: 'none', cursor: 'pointer' }}>⏹ Detener</button>}
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', background: '#1a1a1a', padding: '8px 16px', borderRadius: '16px' }}>
                  <span>⚡</span>
                  <select value={speedMultiplier} onChange={(e) => setSpeedMultiplier(parseFloat(e.target.value))} style={{ background: '#2d2d2d', color: 'white', border: '1px solid #00ff9d', borderRadius: '8px', padding: '6px 12px' }}>
                    {speedOptions.map(opt => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}
                  </select>
                </div>
              </div>
              {error && <div style={{ background: 'rgba(239,68,68,0.2)', border: '1px solid #ef4444', borderRadius: '16px', padding: '12px', marginBottom: '20px', color: '#fecaca' }}>❌ {error}</div>}
              <BarChart key={currentStep} data={currentState.array} comparing={currentState.comparing} swapping={currentState.swapping} />
              <div style={{ marginTop: '20px', textAlign: 'center', color: '#00ff9d' }}>Paso {currentStep + 1} / {steps.length || 1}</div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
              <div style={{ background: 'rgba(20,20,20,0.9)', borderRadius: '20px', padding: '20px', border: '1px solid rgba(0,255,157,0.3)' }}>
                ...
                <p style={{ color: 'white', fontFamily: 'monospace', marginTop: '12px', background: 'rgba(0,0,0,0.5)', padding: '8px', borderRadius: '8px' }}>Array: [{currentState.array?.join(', ')}]</p>
              </div>
              <div style={{ background: 'rgba(20,20,20,0.9)', borderRadius: '20px', padding: '20px', border: '1px solid rgba(0,255,157,0.3)' }}>
                <h3 style={{ color: '#00ff9d' }}>📝 {currentPseudo.title}</h3>
                <pre style={{ color: '#6ee7b7', fontSize: '12px', whiteSpace: 'pre-wrap', background: 'rgba(0,0,0,0.5)', padding: '12px', borderRadius: '8px' }}>{currentPseudo.code}</pre>
              </div>
            </div>
          </>
        )}
